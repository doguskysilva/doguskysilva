<template>
  <section class="mt-4">
    ...
    <div v-if="format === 'card'" class="mt-4 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
      ...
      >
        <NuxtLink :to="postPath(post.path)">
        ...
        <p v-if="post.description" class="mt-2 line-clamp-2 text-sm text-gray-600 dark:text-gray-400">
          {{ post.description }}
        </p>

        <p class="mt-3 text-xs text-gray-400 dark:text-gray-500">
          {{ displayDate(post.date) }}<template v-if="post.body"> · {{ readingTime(post.body) }}</template>
        </p>
    ...
    <ol v-else class="divide-y divide-gray-200 dark:divide-neutral-800">
      <li v-for="post in posts" :key="post.path" class="py-5">
        <NuxtLink :to="postPath(post.path)" class="group block">
...
const { data: allPosts } = await useAsyncData(listingKey, () => {
...
const filteredPosts = computed(() => {
  return (allPosts.value || []).filter((post) => {
    ...
    const matchesPrefix = !props.prefix || post.path.startsWith(props.prefix)
