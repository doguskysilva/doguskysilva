<template>
  <div :class="componentClass">
    <div :class="classes[type.toLowerCase()]?.header || ''">
      <svg
        v-if="type.toLowerCase() === 'tip'"
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 384 512"
        :class="classes[type.toLowerCase()]?.icon || ''"
      >
    ...
    <div :class="classes.content" class="not-prose">
      <slot />
    </div>
  </div>
</template>
...
const defaultClasses: AlertClasses = {
...
const classes = computed(() => (appConfig.ui?.components?.alert?.classes as AlertClasses) || defaultClasses)
const type = computed(() => props.type)

const componentClass = computed(() => {
  const baseClass = classes.value.component
  const typeClass = classes.value[props.type.toLowerCase()]?.component || ''
